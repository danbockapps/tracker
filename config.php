...
function transfer_participant(
   $email_address, /* String: participant's e-mail address */
   $old_class,     /* Integer: transfer participant out of this class */
   $new_class      /* Integer: transfer participant into this class */
) {
   $qr = pdo_seleqt("
      select user_id
      from wrc_users
   ...
   if(count($qr) != 1) {
      $result = "<div class='error'>Error: e-mail address not found.</div>";
   }
   else {
      $result = xferpart($qr[0]['user_id'], $old_class, $new_class, "a");
   }
...
function xferpart(
   ...
   if(
      class_valid($class_for_reassign, $class_source)
      &&
      class_valid($newclass_for_part, $class_source)
   ) {
      if($part_for_reassign >= 0) {
         $cqr = seleqt_one_record("
            ...
            where
               ...
               and class_id = ?
               and class_source = ?
         ", array($part_for_reassign, $newclass_for_part, $class_source));
         /* Is the participant already in that class? */
         if($cqr['count'] == 0) {
            $fqr = seleqt_one_record(
               "
                  select count(*) as count
                  from
                     " . ENR_VIEW . " e
                  ...
                  where
                     ...
                     and not (class_id = ? and class_source = ?)
               ",
               array(
                  $part_for_reassign,
                  $newclass_for_part,
            ...
            if($fqr['count'] == 0) {
               ...
               $sth0 = $dbh0->prepare("
                  ...
                  where
                     ...
                     and class_source = ?
               ");
               $report_success = $sth0->execute(array(
                  $newclass_for_part,
                  $part_for_reassign,
                  $class_for_reassign,
                  $class_source
               ...
               $sth = $dbh->prepare("
                  ...
                  where
                     ...
                     and class_source = ?
               ");
               $enrollment_success = $sth->execute(array(
                  $newclass_for_part,
                  $part_for_reassign,
                  $class_for_reassign,
                  $class_source
               ...
               $sth = $dbh1->prepare("
                  ...
                  where
                     ...
                     and class_source = ?
               ");
               $strategy_success = $sth->execute(array(
                  $newclass_for_part,
                  $part_for_reassign,
                  $class_for_reassign,
                  $class_source
               ...
               if($enrollment_success && $report_success) {
                  return cnf_text("Participant transferred.");
               }
               else {
                  return err_text("There was an error with the database.");
               }
            }
            else {
               return err_text("That participant is already in a class with a
                  start date within 10 weeks of the start date of that class.");
            }
         }
         else {
            return err_text("That participant is already in that class.");
         }
      }
      else {
         return err_text("You did not select a participant.");
      }
   }
   else {
      return err_text("You did not select a valid class.");
   }
}
